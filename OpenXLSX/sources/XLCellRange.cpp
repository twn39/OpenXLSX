...
XLCellRange::XLCellRange(const XMLNode&         dataNode,
    ...
    : m_dataNode(std::make_unique<XMLNode>(dataNode)),
      ...
      m_columnStyles{}
{
    if (m_topLeft.row() > m_bottomRight.row() or m_topLeft.column() > m_bottomRight.column()) {        throw XLInputError(fmt::format("XLCellRange constructor: topLeft ({}) does not point to a lower or equal row and column than bottomRight ({})", topLeft.address(), bottomRight.address()));
    }
}
...
void XLCellRange::fetchColumnStyles()
{
    ...
    while (not col.empty()) {
        ...
        if (minCol > maxCol or !minCol or !maxCol) {            throw XLInputError(fmt::format("column attributes min (\"{}\") and max (\"{}\") must be set and min must not be larger than max", col.attribute("min").value(), col.attribute("max").value()));
        }
