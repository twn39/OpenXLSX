...
namespace OpenXLSX
{
    ...
    size_t XLTableCollection::count() const
    {
        load();
        return m_tables.size();
    }
    XLTable XLTableCollection::operator[](size_t index) const
    {
        load();
        return m_tables.at(index);
    }
    XLTable XLTableCollection::table(std::string_view name) const
    {
        load();
        auto it = std::find_if(m_tables.begin(), m_tables.end(), [&](const XLTable& t) { return t.name() == name; });
        if (it == m_tables.end()) throw XLException("Table not found");
        return *it;
    }
    ...
    uint32_t XLTable::id() const { return xmlDocument().document_element().attribute("id").as_uint(); }
     ...
     */
    const std::vector<std::string_view> TableAttributeOrder = {"id",
                                                               "name",
                                                               "displayName",
                                                               "ref",
                                                               "tableType",
    ...
    void setTableAttribute(XMLNode& node, std::string_view name, std::string_view value)
    {
        ...
        for (const auto& [n, v] : attrs) { node.append_attribute(n.c_str()).set_value(v.c_str()); }
    }

    void XLTable::setName(std::string_view name)
    {
        XMLNode node = xmlDocument().document_element();
        setTableAttribute(node, "name", name);
    }
    std::string XLTable::displayName() const { return xmlDocument().document_element().attribute("displayName").value(); }
    void        XLTable::setDisplayName(std::string_view name)
    {
        XMLNode node = xmlDocument().document_element();
        setTableAttribute(node, "displayName", name);
    }
    ...
    bool XLTable::showRowStripes() const
    ...
    {
        auto col = xmlDocument()
                       .document_element()
                       .child("tableColumns")
                       .find_child_by_attribute("tableColumn", "name", std::string(name).c_str());
    ...
    {
        auto col =
            xmlDocument().document_element().child("tableColumns").find_child_by_attribute("tableColumn", "id", std::to_string(id).c_str());
        return XLTableColumn(col);
    }
...
namespace OpenXLSX
{
    XLTable XLTableCollection::add(std::string_view name, const XLCellRange& range) { return add(name, range.address()); }
}    // namespace OpenXLSX
