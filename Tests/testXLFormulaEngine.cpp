...
namespace
{
    inline const std::string& __global_unique_testXLFormulaEngine_0()
    {
...
{
    XLDocument doc;
    doc.create(__global_unique_testXLFormulaEngine_0(), XLForceOverwrite);
...
TEST_CASE("XLFormulaEngineTEXTFunction", "[XLFormulaEngine][TEXT]")
{
    XLFormulaEngine eng;

    SECTION("Basic Date formats")
    {
        // Excel date: 43926.75 -> 2020-04-05 18:00:00
        REQUIRE(eng.evaluate("=TEXT(43926.75, \"yyyy-mm-dd\")").get<std::string>() == "2020-04-05");
        REQUIRE(eng.evaluate("=TEXT(43926.75, \"yyyy/mm/dd\")").get<std::string>() == "2020/04/05");
        REQUIRE(eng.evaluate("=TEXT(43926.75, \"yyyy-m-d\")").get<std::string>() == "2020-4-5");
    }

    SECTION("Month vs Minute ambiguity (m)")
    {
        REQUIRE(eng.evaluate("=TEXT(43926.75, \"yyyy-mm-dd hh:mm:ss\")").get<std::string>() == "2020-04-05 18:00:00");
        REQUIRE(eng.evaluate("=TEXT(43926.75, \"hh:mm\")").get<std::string>() == "18:00");
        REQUIRE(eng.evaluate("=TEXT(43926.75, \"m\")").get<std::string>() == "4");    // It's month here without context
    }

    SECTION("AM/PM handling")
    {
        REQUIRE(eng.evaluate("=TEXT(43926.75, \"hh:mm AM/PM\")").get<std::string>() == "06:00 PM");
        REQUIRE(eng.evaluate("=TEXT(43926.25, \"hh:mm AM/PM\")").get<std::string>() == "06:00 AM");
    }

    SECTION("Numeric formats MVP")
    {
        REQUIRE(eng.evaluate("=TEXT(1234.567, \"0.00\")").get<std::string>() == "1234.57");
    ...
    SECTION("Multiple Sections: Positive;Negative;Zero;Text") {
        ...
        REQUIRE(eng.evaluate("=TEXT(-1234.5, \"" + fmt + "\")").get<std::string>() == "(1,234.50)");
        REQUIRE(eng.evaluate("=TEXT(0, \"" + fmt + "\")").get<std::string>() == "-");
...
TEST_CASE("XLFormulaEngineDatadrivenQuirksfromCSV", "[XLFormulaEngine][Quirks]")
{
    ...
    while (std::getline(file, line)) {
        ...
        if (expectedTypeStr == "Number") {
            ...
            if (result.type() == XLValueType::Integer) { REQUIRE(result.get<int64_t>() == std::stoll(expectedValueStr)); }
            else {
