...
namespace { 
inline const std::string& __global_unique_testXLRow_0() {
...
TEST_CASE("XLRowTests", "[XLRow]")
{
    ...
    {
        XLDocument doc;
        doc.create(__global_unique_testXLRow_0(), XLForceOverwrite);
        ...
        XLRow copy3;
        copy3 = copy2;
        copy3.setHeight(gsl::narrow_cast<float>(height) * 3.0f);
        REQUIRE(copy3.height() == height * 3.0);
        copy3.setHeight(gsl::narrow_cast<float>(height) * 4.0f);
        REQUIRE(copy3.height() == height * 4.0);

        XLRow copy4;
        copy4 = std::move(copy3);
        copy4.setDescent(gsl::narrow_cast<float>(descent) * 2.0f);
        REQUIRE(copy4.descent() == descent * 2.0f);
        copy4.setDescent(gsl::narrow_cast<float>(descent) * 3.0f);
