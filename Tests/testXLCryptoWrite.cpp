...
TEST_CASE("CryptoEncryptionandDecryptionCycle") {
    SECTION("Standard Encryption Export and Readback") {
        XLDocument doc;
        doc.create(__global_unique_testXLCryptoWrite_0(), XLForceOverwrite);
        ...
        REQUIRE_NOTHROW(doc.saveAs(__global_unique_testXLCryptoWrite_0(), std::string("CyclePass123!"), XLForceOverwrite));
        
        // Verify it can be opened back
        XLDocument doc2;
        REQUIRE_NOTHROW(doc2.open(__global_unique_testXLCryptoWrite_0(), "CyclePass123!"));
    ...
    SECTION("Invalid Password Handling") {
        XLDocument doc;
        doc.create(__global_unique_testXLCryptoWrite_1(), XLForceOverwrite);
        doc.workbook().worksheet("Sheet1").cell("A1").value() = "Secret Data";
        doc.saveAs(__global_unique_testXLCryptoWrite_1(), std::string("CorrectPass"), XLForceOverwrite);

        XLDocument doc2;
        // Opening with the wrong password should throw an XLInternalError (due to HMAC/Verifier mismatch or ZIP signature mismatch)
        REQUIRE_THROWS_AS(doc2.open(__global_unique_testXLCryptoWrite_1(), "WrongPass"), XLInternalError);
    }

    SECTION("Empty Password Handling") {
        XLDocument doc;
        doc.create(__global_unique_testXLCryptoWrite_3(), XLForceOverwrite);
        ...
        REQUIRE_NOTHROW(doc.saveAs(__global_unique_testXLCryptoWrite_3(), std::string(""), XLForceOverwrite));

        XLDocument doc2;
        REQUIRE_NOTHROW(doc2.open(__global_unique_testXLCryptoWrite_3(), ""));
    ...
    SECTION("Extremely Long Password Handling") {
        XLDocument doc;
        doc.create(__global_unique_testXLCryptoWrite_2(), XLForceOverwrite);
        ...
        REQUIRE_NOTHROW(doc.saveAs(__global_unique_testXLCryptoWrite_2(), longPass, XLForceOverwrite));

        XLDocument doc2;
        REQUIRE_NOTHROW(doc2.open(__global_unique_testXLCryptoWrite_2(), longPass));
