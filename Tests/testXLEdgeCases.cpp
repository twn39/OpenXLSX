...
{
    XLDocument doc;
    doc.create(__global_unique_testXLEdgeCases_2(), XLForceOverwrite);
    ...
    SECTION("xml:space=\"preserve\" attribute")
    {
        ...
        doc.open(__global_unique_testXLEdgeCases_2());
        auto wks2 = doc.workbook().worksheet("Sheet1");
        REQUIRE(wks2.cell("A1").value().get<std::string>() == "   Hello World   ");
        doc.close();
    }
    std::filesystem::remove(__global_unique_testXLEdgeCases_2());
}

TEST_CASE("SheetNamingandEscapingEdgeCases", "[EdgeCases][SheetNames]")
{
    XLDocument doc;
    doc.create(__global_unique_testXLEdgeCases_0(), XLForceOverwrite);
    ...
    doc.close();
    std::filesystem::remove(__global_unique_testXLEdgeCases_0());
}

TEST_CASE("StringtoNumberCoercionEdgeCases", "[EdgeCases][Coercion]")
{
    XLDocument doc;
    doc.create(__global_unique_testXLEdgeCases_1(), XLForceOverwrite);
    auto wks = doc.workbook().worksheet("Sheet1");
    ...
    SECTION("Leading Zeros in Strings")
    {
        ...
        doc.open(__global_unique_testXLEdgeCases_1());
        auto wks2 = doc.workbook().worksheet("Sheet1");
        REQUIRE(wks2.cell("A1").value().type() == XLValueType::String);
        REQUIRE(wks2.cell("A1").value().get<std::string>() == "01234");
        doc.close();
    }
    std::filesystem::remove(__global_unique_testXLEdgeCases_1());
}