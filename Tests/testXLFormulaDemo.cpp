...
namespace { 
inline const std::string& __global_unique_testXLFormulaDemo_0() {
...
{
    XLDocument doc;
    doc.create(__global_unique_testXLFormulaDemo_0(), XLForceOverwrite);
    ...
    std::vector<std::string> targetCells = {"D1", "D2", "D3", "D4", "D5", "E1", "E2", "E3", "E4", "F1", "F2", "F3", "G1", "G2", "G3", "G4"};
    ...
    for (const auto& ref : targetCells) {
        ...
        if (result.type() == XLValueType::Float)
            resultStr += std::to_string(result.get<double>());
        else if (result.type() == XLValueType::Integer)
            resultStr += std::to_string(result.get<int64_t>());
        else if (result.type() == XLValueType::Boolean)
            resultStr += (result.get<bool>() ? "TRUE" : "FALSE");
        else if (result.type() == XLValueType::String)
