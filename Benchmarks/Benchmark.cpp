...
TEST_CASE("OpenXLSX Benchmarks", "[.benchmark]")
{
    ...
    SECTION("Read Operations")
    {
        ...
        BENCHMARK("Read Strings")
        {
            ...
            for (auto& row : wks.rows()) {
                ...
                result += std::count_if(values.begin(), values.end(), [](const std::string& v) { return !v.empty(); });
            }
            doc.close();
            return result;
        };
        ...
        BENCHMARK("Read Integers")
        {
            ...
            for (auto& row : wks.rows()) {
                ...
                result += std::accumulate(values.begin(), values.end(), 0ULL, [](uint64_t a, const XLCellValue& b) {
                    return a + b.get<uint64_t>();
                });
            }
            doc.close();
            return result;
    ...
    SECTION("New Features Operations")
    {
        ...
        BENCHMARK("Style Pool - Deduplication")
        {
            ...
            for (size_t i = 0; i < calls; ++i) { styles.findOrCreateStyle(s); }
